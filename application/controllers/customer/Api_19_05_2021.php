    ...
    try
    {
      ...
      if(!empty($order_id)&&!empty($amount)&&!empty($currency) && !empty($admin_id) && !empty($mobile_no))
      {
          ...
          $token=generateToken($api_key,$app_id,$json);
           //print_r(json_decode($token));exit;
          $result=$this->Customer->getVendors($admin_id);
          // print_r($result);exit;
          foreach($result AS $value)
          {

            $brekups=$amount*$value['commission']/100;
            $array['vendorId']=$value['vendorId'];
            $array['amount']=$brekups;
            $paymentSplits[]=$array;
            $brekups=0;
          }
          // print_r($paymentSplits);exit;
          // $json_decode=json_encode($result,JSON_NUMERIC_CHECK);
          $json_decode=json_encode($paymentSplits,JSON_NUMERIC_CHECK);
          // print_r($json_decode);exit;
          $base64_encode=base64_encode($json_decode);
          // print_r($base64_encode);exit;
          ...
          $arry=array('status'=>'1','app_id'=>$app_id,'name'=>$name,'mobile_no'=>$mobile_no,'email_id'=>$email_id,'data'=>json_decode($token),'vendor_split_value'=>$base64_encode);
          $this->response($arry, 200);
...
public function getRestaurantCategory_post()
{
  try
  {
      ...
      if(!empty($admin_id))
      {
          ...
          $result=$this->Customer->getRestaurantCategory($admin_id);
          if(!empty($result))
          {
